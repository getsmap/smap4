...
f = cgi.FieldStorage()
kb = f.getfirst("term", "wrong search parameter")
...
cur = conn.cursor()
sql = "SELECT objektnamn FROM poi_skane_adressregister_apl WHERE (objektnamn like %s AND position(', MALMÖ' in objektnamn) > 0);"
safeParam = (kb+"%", )
